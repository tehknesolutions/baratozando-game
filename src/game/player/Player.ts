...
import { RoachVisual } from './RoachVisual.js';
...
export class Player extends Phaser.Physics.Arcade.Sprite {
  ...
  readonly roachVisual: RoachVisual | null;
  facing: -1 | 1 = 1;
  ...
  constructor(scene: any, x: number, y: number, private readonly inputController: InputController, options: PlayerOptions = {}) {
    ...
    // The actor texture is now only a physics carrier. Premium art lives in RoachVisual.
    ...
    this.checkpoint = { x, y };
    this.roachVisual = premiumVisual ? new RoachVisual(scene, x, y, ROACH_WORLD_SCALE) : null;
    if (this.roachVisual) this.setVisible(false);
  }
  ...
  updatePlayer(nowMs: number, deltaMs: number): void {
    ...
    if (!locked && !dodging && this.mobilityV2 && wallResolution.attached && intent.jumpPressed && wallResolution.wallSide !== 0) { const jump = this.wall.wallJump(nowMs, wallResolution.wallSide as -1 | 1); this.setVelocity(jump.velocityX, jump.velocityY); this.facing = wallResolution.wallSide === -1 ? 1 : -1; this.wallJumpUntil = nowMs + 150; wallJumped = true; wallResolution = { attached: false, wallSide: 0, climbVelocityY: 0, slideVelocityY: Number.POSITIVE_INFINITY }; this.timers.consumeBufferedJump(nowMs); this.timers.consumeCoyote(); }
    ...
    if (hurt) this.setTint(0xff6b53); else this.clearTint(); if (this.damage.isProtected(nowMs) && !this.damage.dead) this.setAlpha(Math.floor(nowMs / 70) % 2 === 0 ? 0.58 : 1); else if (!this.damage.respawning) this.setAlpha(1);
    this.roachVisual?.syncFromActor(this, this.state, nowMs);
  }

  requestRespawn(): void { this.beginRespawn(); }
  private beginRespawn(): void { if (this.respawnScheduled) return; this.respawnScheduled = true; this.damage.beginRespawn(); this.timers.reset(); this.wings.reset(); this.wall.reset(); this.wingFlapUntil = Number.NEGATIVE_INFINITY; this.wallJumpUntil = Number.NEGATIVE_INFINITY; this.setVelocity(0, 0); this.setPosition(this.checkpoint.x, this.checkpoint.y); this.setAlpha(1); this.roachVisual?.syncFromActor(this, 'RESPAWN', this.scene.time.now); this.scene.time.delayedCall(400, () => { this.damage.completeRespawn(); this.deathAt = Number.NEGATIVE_INFINITY; this.respawnScheduled = false; this.setAlpha(1); }); }
}
