...
export class Player extends Phaser.Physics.Arcade.Sprite {
  ...
  takeDamage(nowMs: number, sourceX: number): void {
    const result = this.damage.damage(nowMs); if (result === 'IGNORED') return;
    const knockDirection = this.x < sourceX ? -1 : 1; this.setVelocity(knockDirection * 145, -145);
    if (result === 'DEATH') this.deathAt = nowMs;
  }
  kill(nowMs: number, sourceX: number): void {
    const result = this.damage.kill(nowMs); if (result === 'IGNORED') return;
    const knockDirection = this.x < sourceX ? -1 : 1; this.setVelocity(knockDirection * 180, -165); this.deathAt = nowMs;
  }
  ...
  updatePlayer(nowMs: number, deltaMs: number): void {
    ...
    let wallJumped = false; let wingFlapped = false;
    if (!locked && !dodging && this.mobilityV2 && wallResolution.attached && intent.jumpPressed && wallResolution.wallSide !== 0) {
      const jump = this.wall.wallJump(nowMs, wallResolution.wallSide as -1 | 1); this.setVelocity(jump.velocityX, jump.velocityY);
      this.facing = wallResolution.wallSide === -1 ? 1 : -1; this.setFlipX(this.facing < 0); this.wallJumpUntil = nowMs + 150; wallJumped = true;
      wallResolution = { attached: false, wallSide: 0, climbVelocityY: 0, slideVelocityY: Number.POSITIVE_INFINITY }; this.timers.consumeBufferedJump(nowMs); this.timers.consumeCoyote();
    } else if (!locked && !dodging && this.timers.canConsumeBufferedJump(nowMs) && this.timers.canCoyoteJump(nowMs)) {
      if (this.timers.consumeBufferedJump(nowMs)) { this.setVelocityY(this.movement.jumpVelocity); this.timers.consumeCoyote(); }
    } else if (!locked && !dodging && this.mobilityV2 && intent.jumpPressed && !grounded) {
      if (this.wings.tryFlap(nowMs, true)) { this.setVelocityY(this.wings.flapVelocity()); this.wingFlapUntil = nowMs + 115; wingFlapped = true; }
    }
    ...
    if (!locked) {
      if (intent.moveX !== 0) { this.facing = intent.moveX; this.setFlipX(this.facing < 0); }
      if (dodging) this.setVelocityX(this.facing * this.movement.dodgeSpeed);
      else if (wallResolution.attached) this.setVelocityX(0);
      else { const dt = Math.min(deltaMs, 50) / 1000; const target = intent.moveX * (intent.run ? this.movement.runSpeed : this.movement.walkSpeed); const current = body.velocity.x as number;
        if (grounded) { const accel = intent.moveX === 0 ? this.movement.groundDeceleration : this.movement.groundAcceleration; this.setVelocityX(approach(current, target, accel * dt)); }
    ...
    if (hurt) this.setTint(0xff6b53); else this.clearTint();
    if (this.damage.isProtected(nowMs) && !this.damage.dead) this.setAlpha(Math.floor(nowMs / 70) % 2 === 0 ? 0.58 : 1); else if (!this.damage.respawning) this.setAlpha(1);
    this.animator.update(this.state, nowMs);
  }

  requestRespawn(): void { this.beginRespawn(); }
  private beginRespawn(): void {
    if (this.respawnScheduled) return; this.respawnScheduled = true; this.damage.beginRespawn(); this.timers.reset(); this.wings.reset(); this.wall.reset();
    this.wingFlapUntil = Number.NEGATIVE_INFINITY; this.wallJumpUntil = Number.NEGATIVE_INFINITY; this.setVelocity(0, 0); this.setPosition(this.checkpoint.x, this.checkpoint.y); this.setAlpha(1);
    this.scene.time.delayedCall(400, () => { this.damage.completeRespawn(); this.deathAt = Number.NEGATIVE_INFINITY; this.respawnScheduled = false; this.setAlpha(1); });
  }
}
